...
<div class="main-content-wrap">
    ...
    <div class="wg-box">
        <form class="form-new-product form-style-1" action="{{route('admin.slide.update')}}" method="POST" enctype="multipart/form-data">
            ...
            <fieldset>
                <div class="body-title">Upload background images <span class="tf-color-1">*</span>
                </div>
                <div class="upload-image flex-grow">
                    @if($slide->bgimage)
                    <div class="item" id="bgimage">
                        <img src="{{asset('uploads/slides/bgimage')}}/{{$slide->bgimage}}" class="effect8" alt=""/>
                    </div>
                    @endif
                    <div class="item up-load">
                        <label class="uploadfile" for="bgFile">
                            <span class="icon">
                                <i class="icon-upload-cloud"></i>
                            </span>
                            <span class="body-text">Drop your images here or select <span class="tf-color">click to browse</span></span>
                            <input type="file" id="bgFile" name="bgimage"/>
                        </label>
                    </div>
...
@push('scripts')
    <script>
        ...
        $("#bgFile").on("change",function(e){
                const photoInp = $("#bgFile");
                const [file] = this.files;
                if(file){
                    $("#bgimage img").attr('src', URL.createObjectURL(file));
                    $("#bgimage").show();
                }
            });
    </script>
